...
const SCF_PERMISSION_SECTIONS=[
  ...
  {sec:'Mua hàng & bảo dưỡng',pages:[['nccgoods','Nhà cung cấp hàng hóa'],['purchasegoods','Mua hàng hóa'],['fuelpurchases','Mua xăng dầu'],['maint_vehicle','Bảo dưỡng xe'],['maint_machine','Bảo dưỡng máy']]},
...
const DEFAULT_PERMISSION_PROFILES={
  ...
  production_manager:scfProfile('production_manager','Quản lý sản xuất','manager','Sản xuất',[...SCF_PROFILE_COMMON,'advances','rewards','employee_errors','workreport_sx','workreport_total','process_bun','process_pho','process_banhcuon','materials','assets','products','prodshifts','deliveryrules','workcats','intem','orderdetail','maint_machine','prodsummary','prodorders','stock'],['attendance_report','workreport_total','materials','assets','products','orderdetail']),
...
const PERMISSION_PROFILE_ORDER=['director','accounting_manager','accounting_staff','production_manager','production_staff','driver'];
function normalizePermissionProfiles(value){
  ...
  return Object.fromEntries(PERMISSION_PROFILE_ORDER.map(id=>{
    ...
    const levels={};permissions.forEach(page=>{const level=raw.permLevels?.[page]||base.permLevels?.[page]||(base.role==='manager'?'rwd':'rw');levels[page]=['r','rw','rwd'].includes(level)?level:'r';});
...
function PermissionSettingsTab({profiles,setProfiles,employees,setEmployees,currentUser}){
  ...
  const setLevel=(page,level)=>setDraft(prev=>{const permissions=level==='none'?prev.permissions.filter(item=>item!==page):[...new Set([...prev.permissions,page])];const permLevels={...prev.permLevels};if(level==='none')delete permLevels[page];else permLevels[page]=level;return{...prev,permissions,permLevels};});
  const save=()=>{setProfiles(prev=>({...normalizePermissionProfiles(prev),[selected]:{...draft,permissions:[...draft.permissions],permLevels:{...draft.permLevels}}}));window.showToast('Đã lưu quyền mặc định cho '+draft.label+'.','success');};
